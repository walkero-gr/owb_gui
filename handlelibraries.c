...
void OpenLibs()
{
    ...
    if((P96Base = IExec->OpenLibrary("Picasso96API.library", 2L)))
	{
    ...
    Catalog = ILocale->OpenCatalog(NULL, (STRPTR )"OBW_gui.catalog",
...
void CleanExit(char *str)
{
	...
	if(obj[OID_WindowObject4])  IIntuition->DisposeObject(obj[OID_WindowObject4]);

    Ep_CloseWindow_Window3();
	if(obj[OID_WindowObject3])		IIntuition->DisposeObject(obj[OID_WindowObject3]);
	
    Ep_CloseWindow_Window2();
	if(obj[OID_WindowObject2])		IIntuition->DisposeObject(obj[OID_WindowObject2]);
